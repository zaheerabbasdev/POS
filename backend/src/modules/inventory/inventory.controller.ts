...
export const getStockHistory = asyncHandler(async (req: Request, res: Response) => {
  const shopId = getShopId(req);
  const history = await inventoryService.getStockHistory(shopId, req.params.productId as string);
...
export const createAdjustment = asyncHandler(async (req: Request, res: Response) => {
  ...
  const inventory = await inventoryService.createAdjustment(shopId, req.body, req.user.id);
