...
export const listUsers = asyncHandler(async (req: Request, res: Response) => {
  ...
  const { data, pagination } = await userService.listUsers(shopId, req.validatedQuery as unknown as ListUsersInput);
  sendPaginated(res, data, pagination);
});

export const getUser = asyncHandler(async (req: Request, res: Response) => {
  const shopId = getShopId(req);
  const user = await userService.getUserById(shopId, req.params.id as string);
  sendSuccess(res, user);
});

export const createUser = asyncHandler(async (req: Request, res: Response) => {
  const shopId = getShopId(req);
  const user = await userService.createUser(shopId, req.body);
...
export const updateUser = asyncHandler(async (req: Request, res: Response) => {
  const shopId = getShopId(req);
  const user = await userService.updateUser(shopId, req.params.id as string, req.body);
...
export const deleteUser = asyncHandler(async (req: Request, res: Response) => {
  ...
  const shopId = getShopId(req);
  await userService.deleteUser(shopId, req.params.id as string, req.user.id);
